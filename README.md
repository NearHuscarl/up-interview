...
# run each command in a separate terminal to serve both servers
yarn start-api
yarn start-web
```
...
- CSS
  - Components inside `weather-forecast-web/src/ui-base/` are automatically generated, do not edit them directly.
  - Global styles, tailwind.config.js, postcss.config.js
- NX
  - How the project is wired up in a monorepo using `project.json` and `nx.json`
  - eslint, tsconfig, prettierrc...
